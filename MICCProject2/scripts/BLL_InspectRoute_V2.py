...
from PyQt6.QtWidgets import QApplication, QDialog, QAbstractItemView, QTableWidgetItem, QMessageBox, QHeaderView, QMainWindow, QSizePolicy,QVBoxLayout,QSplitter,QWidget,QHBoxLayout,QWidget
...
class BLL_InspectRoute(QDialog):
              ...
              <body>
               ...
               // 初始化地图
                ...
                window.loadWalkingRoute = function(points) {{
                    ...
                    // 3. 构造带途经点的配置对象（核心修复！）
                    var searchOption = {{
                        origin: new AMap.LngLat(points[0].lng, points[0].lat),  // 起点
                        destination: new AMap.LngLat(points[points.length-1].lng, points[points.length-1].lat),  // 终点
                        waypoints: points.length > 2 ? points.slice(1, -1).map(p => new AMap.LngLat(p.lng, p.lat)) : []  // 途经点
                    }};
                    //alert("searchOption详情" + JSON.stringify(searchOption, null, 2));

                    // 2. 显式加载Walking插件（关键！）
                    AMap.plugin('AMap.Walking', function() {{                        
                    // 3. 插件加载完成后，才能实例化AMap.Walking
                        var walking = new AMap.Walking({{
                            map: map
                        }});
                        alert("规划结果：");
                        // 5. 发起步行路线规划
                        walking.search({{searchOption}}, function(status, result) {{
                        alert("规划结果：" + status);
                            if (status === 'complete') {{
                                // 提取路径点，手动绘制路线（兜底）
                                var pathAll = [];
                                result.routes[0].steps.forEach(step => pathAll = pathAll.concat(step.path));
                                ...
                                map.setFitView([polyline].concat(markers));
                                alert("步行路线加载成功！路径点数量：" + pathAll.length);
                            }} else {{
                                // 规划失败，绘制直线兜底
                                ...
                                alert("步行路线规划失败：" + result.info + "，已绘制直线");
                            }}
                        }});
                    }});

                    // 插件加载失败的兜底
                    AMap.plugin('AMap.Walking', function() {{}}, function(err) {{
                        console.error("Walking插件加载失败：", err);
                        alert("步行路线插件加载失败，请检查Key权限！");
                    }});
                }};
            </script>
        </body>
    ...
    def load_walking_route(self):
        ...
        recordlist = self.db.fetch_all(
            "select ip.Longitude, ip.Latitude, ip.PointName from InspectRoutePoint rp, InspectPoint ip where rp.PointId = ip.PointId and rp.RouteId=" + str(self.route_id) +" order by SortNo")
        for row, record in enumerate(recordlist):
            position = {
