...
from modules.task_management.ui.generated.TargetRecognition import Ui_Form


class TargetRecognitionPage(QtWidgets.QWidget, Ui_Form):
    ...
    def apply_titles(self) -> None:
        ...
        if hasattr(self, "lblResultTitle"):
            self.lblResultTitle.setText("\u8bc6\u522b\u7ed3\u679c")
        if hasattr(self, "lblRecordTitle"):
            self.lblRecordTitle.setText("\u8bc6\u522b\u8bb0\u5f55")
    ...
    def apply_demo_content(self) -> None:
        self.set_map_preview("Detection area - Zone A")
        self.set_result_preview("Targets: 3 | Vehicles: 1")
        self.set_record_preview("Latest record - 10:24")
        self.set_task_info(
            name_text="Detect-04",
            type_text="????",
            traveled_text="0.5 km",
            progress_text="58%",
        )
    ...
    def set_result_preview(self, text: str) -> None:
        placeholder = getattr(self, "resultPlaceholder", None)
        if not placeholder:
            return
        if not hasattr(self, "_result_preview_label"):
            self._result_preview_label = self._replace_placeholder(placeholder, FIF.INFO, text)
        else:
            self._result_preview_label.setText(text)

    def set_record_preview(self, text: str) -> None:
        placeholder = getattr(self, "recordPlaceholder", None)
        if not placeholder:
            return
        if not hasattr(self, "_record_preview_label"):
            self._record_preview_label = self._replace_placeholder(placeholder, FIF.HISTORY, text)
        else:
            self._record_preview_label.setText(text)
